...
$_POST["CompanyName"] = (isset($_POST["CompanyName"])) ? $_POST["CompanyName"] : "";
$_POST["companyLoc"] = (isset($_POST["companyLoc"])) ? $_POST["companyLoc"] : "";
$_POST["CompanyUrl"] = (isset($_POST["CompanyUrl"])) ? $_POST["CompanyUrl"] : "";
$_POST["companyEmail"] = (isset($_POST["companyEmail"])) ? $_POST["companyEmail"] : "";

if (empty($_SESSION["user"])) {
  if (isset($_GET["task"])) {
    if ($_GET["task"] == "verify") {
      //Check if all the datat exists
      if (isset($_POST["firstname"]) && isset($_POST["lastname"]) && isset($_POST["email"]) && isset($_POST["pwd"])) {

        //Remove all the unwanted chars from the user input
        $_POST["email"] = filter_var($_POST["email"], FILTER_SANITIZE_EMAIL);
        $_POST["firstname"] = filter_var($_POST["firstname"], FILTER_SANITIZE_STRING);
        $_POST["lastname"] = filter_var($_POST["lastname"], FILTER_SANITIZE_STRING);
        $_POST["pwd"] = filter_var($_POST["pwd"], FILTER_SANITIZE_STRING);
        $is_employer = (isset($_POST['isEmployer']) && $_POST['isEmployer'] == "1") ? true : false;

        //Validate Email
        if (filter_var($_POST["email"], FILTER_VALIDATE_EMAIL)) {
          $user = new \controllers\User();
          if ($user->add(strval($_POST["firstname"]), strval($_POST["lastname"]), strval($_POST["email"]), strval($_POST["pwd"]), "server/default/noAvatar.png", intval($is_employer))) {
            if (!empty($user->get($_POST["email"], $_POST["pwd"]))) {
              $_SESSION["user"] = $user->get($_POST["email"], $_POST["pwd"]);
              if ($is_employer) {
                if (isset($_POST["CompanyName"]) && isset($_POST["companyLoc"]) && isset($_POST["CompanyUrl"]) && isset($_POST["companyEmail"])) {
                  //Remove all the unwanted chars from the user input
                  $_POST["CompanyName"] = filter_var($_POST["CompanyName"], FILTER_SANITIZE_STRING);
                  $_POST["companyLoc"] = filter_var($_POST["companyLoc"], FILTER_SANITIZE_STRING);
                  $_POST["CompanyUrl"] = filter_var($_POST["CompanyUrl"], FILTER_SANITIZE_URL);
                  $_POST["companyEmail"] = filter_var($_POST["companyEmail"], FILTER_SANITIZE_EMAIL);

                  $Company = new \controllers\Company();
                  if ($Company->add(intval($_SESSION["user"]["user_id"]), strval($_POST["CompanyName"]), strval($_POST["companyEmail"]), strval($_POST["CompanyUrl"]), strval($_POST["companyLoc"]), "templates/img/nologo.png"))
                    $_SESSION["company"] = $Company->get($_SESSION["user"]["user_id"]);
                }
              }
...
<div class="container pt-2 pb-2 showMain one-content">
  <div class="row backgroundSvg">
    <svg class="col my-auto poster" id="b59c9f89-c93e-413f-a14b-776784e615e2" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="1075.51587" height="646.89913" viewBox="0 0 1075.51587 646.89913">
      ...
      <ellipse cx="536" cy="625.89913" rx="536" ry="21" fill="#d0cde1"></ellipse>
      <rect x="184" y="248.03055" width="612" height="364.87" fill="#c82333"></rect>
      <path d="M397.70209,374.581a146.35424,146.35424,0,0,1-146.32,143.97c-1.72,0-3.43-.03-5.14-.09V374.581Z" transform="translate(-62.24207 -126.55043)" fill="#28a745"></path>
      <path d="M582.17908,739.45064H523.9723a31.00847,31.00847,0,0,1,58.20678,0Z" transform="translate(-62.24207 -126.55043)" fill="#28a745"></path>
      <path d="M858.24207,550.72094v122.9a61.52336,61.52336,0,0,1,0-122.9Z" transform="translate(-62.24207 -126.55043)" fill="#28a745"></path>
      <rect x="279.45927" y="380.6149" width="422.14211" height="36.06239" rx="18.03118" fill="#fff"></rect>
      <rect x="279.45927" y="451.67903" width="422.14211" height="36.06239" rx="18.03118" fill="#fff"></rect>
      <rect x="553.59711" y="522.74315" width="148.00427" height="36.06239" rx="18.03118" fill="#fff"></rect>
      <path d="M955.29387,238.67007s16.82875,6.54451,17.76368,28.04791-4.67465,81.33893-4.67465,81.33893,20.56846,29.91777,4.67465,38.33214-17.76368-33.65749-17.76368-33.65749l-18.6986-88.81837S947.81443,237.73514,955.29387,238.67007Z" transform="translate(-62.24207 -126.55043)" fill="#a0616a"></path>
      <path d="M783.26671,307.85491s-40.202,26.178-42.07187,5.60958S753.34894,212.492,753.34894,212.492s1.86986-34.59242,21.50339-26.178-5.60958,37.39721-5.60958,37.39721l2.80479,65.44511,18.69861-.93493Z" transform="translate(-62.24207 -126.55043)" fill="#a0616a"></path>
      ...
      <circle cx="733.17873" cy="44.80466" r="31.78763" fill="#a0616a"></circle>
      <path d="M817.85913,167.61537s24.30818,20.56847,41.13693,20.56847S834.68787,226.516,834.68787,226.516l-36.46228-32.72256Z" transform="translate(-62.24207 -126.55043)" fill="#a0616a"></path>
    ...
    <form class="col my-auto needs-validation" method="POST" action="?act=signin&task=verify">
      <h3>Sign In</h3>
      <div class="form-row">
      ...
      <div class="form-row">
        <div class="form-group col-md-6">
          ...
          <input type="email" name="email" class="form-control" id="inputEmail" placeholder="ex: test@test.com" value="<?= $_POST["email"] ?>" required>
        </div>
        <div class="form-group col-md-6">
      ...
      <div class="form-group">
        <div class="form-check">
          <input class="form-check-input" name="isEmployer" type="checkbox" id="emloyerCheck" value="1">
          <label class="form-check-label" for="gridCheck">
            I am an employer.
          </label>
        </div>
      </div>
      <div class="d-none" id="companyInfos">
        <div class="form-row">
          <div class="form-group col-md-6">
            <label for="fname">Company name</label>
            <input type="text" name="CompanyName" class="form-control" id="CompanyName" value="<?= $_POST["CompanyName"] ?>">
          </div>
          <div class="form-group col-md-6">
            <label for="lasttname">Company location</label>
            <input type="text" name="companyLoc" class="form-control" id="companyLoc" value="<?= $_POST["companyLoc"] ?>">
          </div>
        </div>
        <div class="form-row">
          <div class="form-group col-md-6">
            <label for="fname">Company url</label>
            <input type="text" name="CompanyUrl" class="form-control" id="CompanyUrl" value="<?= $_POST["CompanyUrl"] ?>">
          </div>
          <div class="form-group col-md-6">
            <label for="lasttname">Company email</label>
            <input type="Email" name="companyEmail" class="form-control" id="companyEmail" value="<?= $_POST["companyEmail"] ?>">
          </div>
        </div>
      </div>
      <button type="submit" class="btn btn-danger">Sign in</button>
      <p>Already have an account ? <a class="text-x-large a-trans-400" href="?act=login">Log in</a></p>
    </form>
  </div>
</div>

<script>
  $("#emloyerCheck").change(function() {
    if (this.checked) {
      $("#companyInfos").removeClass("d-none");
      $("#companyInfos input").prop("required", true);
    } else {
      $("#companyInfos").addClass("d-none");
      $("#companyInfos input").removeAttr("required");
    }
  });
</script>